...
public class DataCollectionService extends Service implements
	...
	// Unit in seconds
	private static final int EXPONENTIAL_BACKOFF_UPPER_BOUND = 30 * 60; // 30
																		// minutes
	private static final int WALKING_DELAY = 10; // The minimum delay (seconds)
													// between each wifi sensor
													// sample when the user is
													// walking (on foot)
	private static final int MINIMAL_DELAY = WALKING_DELAY; // The absolute
															// minimum delay in
															// all cases between
															// each wifi sensor
															// sample

	private static final float REQUIRED_LOCATION_ACCURACY = 15; // 68% (1
																// standard
																// deviation)
																// that it is
																// within 15
	...
	private Long lastActivityRecognitionUpdate; // timestamp of last activity
												// recognition update
	 ...
	 * Create an instance of ActivityRecognitionService to listen to the
	 * response.
	 */
	private void requestActivityUpdate() {
		Intent intent = new Intent(this, ActivityRecognitionService.class);
		callbackIntent = PendingIntent.getService(getApplicationContext(), 0,
				intent, PendingIntent.FLAG_UPDATE_CURRENT);
		mActivityRecognitionClient.requestActivityUpdates(
				ACTIVITY_UPDATE_DELAY, callbackIntent);
		startService(intent);
	}

	/**
	 * The broadcast receiver listens to: </br> - Wifi network scan results.
	 * </br> This is used to record encounters/wifi sensor measurements. </p> -
	 * Network state change </br> This is used to record interactions. When ever
	 * a user successfully/unsuccessfully connects to an open network we want to
	 * record it.
	 * <p>
	 * - Activity recognition data </br> Received from the
	...
	private void initialiseBroadcastReceiver() {
		receiver = new BroadcastReceiver() {
			...
			public void onReceive(Context c, Intent intent) {
				if (intent.getAction().compareTo(
					...
					networkStateChangedReceiverAction(intent);
						...
						.getAction()
						.compareTo(
								"sg.edu.astar.i2r.sns.activity.ACTIVITY_RECOGNITION_DATA") == 0
			...
			/**
			 * A network state change is detected so we want to record the
			...
			public void networkStateChangedReceiverAction(Intent intent) {
				...
				locationKey = databaseHelper
						.matchOrCreateLocationRecord(location);
				if (locationKey != Constant.INVALID)
					accuracy = location.getAccuracy();

				recordStateInformation(intent, networkInfo, timestamp,
						locationKey, accuracy, status);
			}

			public void recordStateInformation(Intent intent,
					NetworkInfo networkInfo, long timestamp, long locationKey,
					double accuracy, int status) {
				long accessPointKey = Constant.INVALID;
				String ssid = null, bssid = null;

				if (NetworkInfo.DetailedState.CONNECTED.compareTo(networkInfo
						.getDetailedState()) == 0) { // Wifi connected
					...
					accessPointKey = databaseHelper.getAccessPointPrimaryKey(
							ssid, bssid);
					...
					databaseHelper.createInteractionRecord(wifiInfo, timestamp,
							accuracy, accessPointKey, locationKey, status);
				} else if (NetworkInfo.DetailedState.CONNECTING
						.compareTo(networkInfo.getDetailedState()) == 0) { // Wifi
																			// connected
					Log.d(TAG, "connecting");
				} else if (NetworkInfo.DetailedState.BLOCKED
						.compareTo(networkInfo.getDetailedState()) == 0) {
					Log.d(TAG, "blocked");
				} else if (NetworkInfo.DetailedState.FAILED
						.compareTo(networkInfo.getDetailedState()) == 0) {
					Log.d(TAG, "failed");
				} else if (NetworkInfo.DetailedState.DISCONNECTED
						.compareTo(networkInfo.getDetailedState()) == 0) {
					Log.d(TAG, "disconnected");
					// } else if
					// (NetworkInfo.DetailedState.VERIFYING_POOR_LINK.compareTo(networkInfo.getDetailedState())
					// == 0) {
				}
			}
			...
			public void activityRecognitionReceivedAction(Intent intent) {
				...
				if (intent.getStringExtra("Activity").compareTo("On Foot") == 0
						&& intent.getIntExtra("Confidence", Constant.INVALID) > 0.75) {
					activityType = DetectedActivity.ON_FOOT;
					long now = System.currentTimeMillis();

					if (lastActivityRecognitionUpdate == null
							|| ((now - lastActivityRecognitionUpdate) > (MINIMAL_DELAY * 1000))) {
						recordEncounterDelay = WALKING_DELAY; // Need to
																// synchronise
	...
	//
	// if (location != null) {// && location.getAccuracy() <
	// LOCATION_ACCURACY_TOLERANCE) {
	// primaryKey = databaseHelper.matchOrCreateLocationRecord(location);
	// if (primaryKey == Constant.INVALID) {
	// Log.d(TAG,
	// "couldn't find or create location record - Not meant to occur");
	// }
	// }
	// return primaryKey;
	// }
	...
	private class ScanResultsRecorder extends AsyncTask<String, Void, String> {
		...
		private void createEncounterEntries() {
			...
			encounterKey = databaseHelper.createEncounterRecord(timestamp,
					accuracy, locationKey);
			...
			for (ScanResult result : wifiList) {
				if (WifiUtils.isRequired(result)) {
					...
					accessPointKey = databaseHelper
							.matchOrCreateAccessPointRecord(result);
					databaseHelper.createEncounterWithAccessPointRecord(
							encounterKey, accessPointKey, result.level);
					// if (haveNotEncountered(result)) allEncounteredBefore =
					// false;
				}
			...
			// recordInteraction(location.getAccuracy(), locationKey,
			// timestamp);
			// }
		}
		...
		// try {
		// List<Address> list =
		// geocoder.getFromLocation(testLocation.getLatitude(),
		...
		private void performExponentialBackoff() {
			...
			// Log.d(TAG, "Exponential backoff result: " +
			// recordEncounterDataDelay);
		}

		private boolean haveNotEncountered(ScanResult result) {
			cursor = databaseHelper.getAccessPointRecord(result.SSID,
					result.BSSID);
			return (cursor != null && cursor.moveToFirst()) ? false : true;
		}

		public void recordInteraction(float accuracy, long locationKey,
				long timestamp) {
			...
			if (ssid != null && bssid != null) {
				...
				if (cursor != null && cursor.moveToFirst()) {
					accessPointKey = cursor.getLong(cursor
							.getColumnIndexOrThrow(AccessPointTable.COLUMN_ID));
					databaseHelper.createInteractionRecord(wifiInfo, timestamp,
							accuracy, accessPointKey, locationKey, status);
				} else {
					Log.d(TAG,
							"Can't find " + ssid + " - " + wifiInfo.getBSSID());
				}
	...
	public void onDestroy() {
		...
		try {
			...
			// This is a hack since you can't check if it was it was initially
			// registered (could use boolean flag)
		}
