...
public class CollectionDatabaseHelper extends SQLiteOpenHelper {
	...
	public boolean createRecordReport(ContentValues values, Uri tableUri, String columnId) {	
		context.getContentResolver().insert(tableUri, values);		
		return true;
	}
	public boolean updateRecordReport(ContentValues values, Uri tableUri, String selection,String[] selectionArgs) {	
		context.getContentResolver().update(tableUri, values,selection,selectionArgs);		
		return true;
	}
	...
	/*((new Date().getTime()), quality,  Long.parseLong(telephonyMgr.getDeviceId()),
			currentNetwork.getBSSID(), ssid, login_required, name, address,latitude,longitude,granularity);*/
	public void saveReport(Long timestamp, String quality, String idDevice, String bssid, String ssid, String login_required,
			String name, String address,String latitude, String longitude,String granularity,String floor_room){
		...
		createRecordReport(reportValues, CollectionContentProvider.CONTENT_URI_TEMPREPORT, TempReportTable.COLUMN_ID);
		...
		// Check if the Place record already exists
				...
				if (cursor.moveToFirst()) {
					...
					for (int i=0; i< nbRows; i++) {
						...
						updateRecordReport(reportValues, CollectionContentProvider.CONTENT_URI_REPORT, selection, selectionArgs);
						cursor.moveToNext();
					}
					cursor.close();
				} else {
					...
					createRecordReport(reportValues, CollectionContentProvider.CONTENT_URI_REPORT, ReportTable.COLUMN_ID);
					Toast.makeText(context, "HOTSPOT SAVED", Toast.LENGTH_LONG).show();
				}*/
	}
	// databaseHelper.saveData("bssid", "ssid", "rating","latitude","longitude","radius", "name", "address", "floor", "room");
	/**
	 * @param SSID
	 * @param BSSID
	...
	public Cursor getAccessPointInformation(String ssid, String bssid) {
		...
		String selection = ReportTable.COLUMN_SSID + "=?";// AND "
				// + ReportTable.COLUMN_BSSID + " LIKE ?"; //AND " + AccessPointTable.COLUMN_BSSID + "=?";
//		Log.d(TAG, ssid);
//		Log.d(TAG, bssid);
		String[] selectionArgs = {ssid};//, bssid};
		String[] projection = {
				ReportTable.COLUMN_ID, ReportTable.COLUMN_SSID,
				ReportTable.COLUMN_BSSID,
		...
		cursor = context.getContentResolver().query(uri, projection, "ssid=?",new String[]{ssid}, null);
...
//		Log.d(TAG, bssid);
		...
		int row = cursor.getCount();
		if (row == 0 ) {
			selection = ReportTable.COLUMN_ADDRESS + " LIKE ?";// AND "
	...
	public Cursor getNearbyAccessPoint(double currentLatitude, double currentLongitude, double radius) {
		...
		String[] projection = {ReportTable.COLUMN_SSID, ReportTable.COLUMN_BSSID, 
				ReportTable.COLUMN_ACCURACY, ReportTable.COLUMN_QUALITY, ReportTable.COLUMN_LOGIN,
		...
		do {
			log.addRecordToLog("BSSID="+cursor.getColumnIndexOrThrow(ReportTable.COLUMN_BSSID)+ " Lat="
					+cursor.getColumnIndexOrThrow(ReportTable.COLUMN_LATITUDE)
					+"  Long="+cursor.getColumnIndexOrThrow(ReportTable.COLUMN_LONGITUDE));
	...
	public void saveData(String bssid, String ssid, String rating,String latitude, String longitude,
			String radius, String name, String address, String floor, String room, boolean login) {
